...
if (isset($_GET['q'])) {
	$id = $_GET['q'];
	$sql = 'select place.*,Name from place inner join city on place.CityID = city.CityID where PlaceID ='.$id ;
...
<body>
	...
	<div class="main" style = "min-height: 70vh;">
		<div class="container">
			...
			<div class="row">
				<h3 class="mb-3">Thông tin địa điểm</h3>
			...
			<div class="comment">
				...
				<div class="row">
					<div class="col-sm-12 col-md-8 offset-md-2 col-lg-6 offset-lg-3 col-xl-4 offset-xl-4">
						<?php 
						$sql = 'select Rating from place where placeID ='.$id ;
						$resultOverviewRating = executeSingleResult($sql);
						$sqlTotal = 'SELECT COUNT(CommentID) as number FROM comment WHERE PlaceID = '.$id ;
						$resultTotalRating = executeSingleResult($sqlTotal);
						$sql1 = 'SELECT COUNT(CommentID) as number FROM comment WHERE PlaceID = '.$id.' and RatingStar = 1';
						$sql2 = 'SELECT COUNT(CommentID) as number FROM comment WHERE PlaceID = '.$id.' and RatingStar = 2';
						$sql3 = 'SELECT COUNT(CommentID) as number FROM comment WHERE PlaceID = '.$id.' and RatingStar = 3';
						$sql4 = 'SELECT COUNT(CommentID) as number FROM comment WHERE PlaceID = '.$id.' and RatingStar = 4';
						$sql5 = 'SELECT COUNT(CommentID) as number FROM comment WHERE PlaceID = '.$id.' and RatingStar = 5';
				...
				<div class="row">
					<div class="col-3 mb-3">
						<a href="danhgiadiadiem.php?id=<?php echo $id?>" class="btn btn-dark">
							Viết đánh giá
				...
				<div class="row">
					<div class="col-md-12">
						<div class="card" style="cursor: unset">
							...
							<?php 
							$sql = 'select comment.*,Username,Avatar from comment inner join account on UserID = AccountID where PlaceID ='.$id.' order by CommentID DESC';
				...
				<!-- End Comment -->
				<!-- More places -->
				<div class="more-places mb-5">
					<div class="row justify-content-center">
						<div class="col-lg-6">
							<div class="text-center my-5">
								<h3>Các địa điểm gần đây</h3>
							</div>
						</div>
					</div>
					<div class="row">
						<?php 
						$sql_more = 'select * from place where CityID ='.$result['CityID'].' and PlaceID not in ('.$result['PlaceID'].') order by PlaceID DESC limit 6';
						$result_more = executeResult($sql_more);
						foreach ($result_more as $key => $value) {
							?>
							<div class="col-lg-3 col-md-4">
								<a href="?q=<?php echo $value['PlaceID'] ?>" class="text-dark text-decoration-none">
									<div class="card mb-4">
